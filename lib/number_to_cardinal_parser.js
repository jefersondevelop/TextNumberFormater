...
const cardinalParser = () => {
    ...
    function processNumberToReturn(digits        )         {
        ...
        if (mileMillions > 0) result += getCardinalString(mileMillions, result.length > 0).toString() + 'Mil ';
        if (millions > 0) result += getCardinalString(millions, result.length > 0).toString();

        if (mileMillions === 0 && millions === 1) result += 'Millón ';
        else if (mileMillions > 0 || millions > 0) result += 'Millones ';

        if (miles > 0) result += getCardinalString(miles, result.length > 0).toString() + 'Mil ';
        ...
        if (units > 0) result += getCardinalString(units, result.length > 0).toString();
    ...
    function getUnitsCardinalString(units        , tens        , hundreds        , separator        )         {
        ...
        switch (units) {
            case 0:
                break;
            case 1:
                if (tens === 2 || hundreds > 0)
                    unitsCardinalString += 'Ún ';
                else
                    unitsCardinalString += (!separator) ? 'Un ' : 'Uno ';
                break;
            case 2:
                if (!separator)
