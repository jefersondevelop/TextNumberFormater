...
const cardinalParser = () => {
    ...
    function processNumberToReturn(digits        )         {
        ...
        if (miles > 0) {
            console.log('result => ', miles === 1 ? 'Un Mil ' : getCardinalString(miles, result.length > 0) + 'Mil ')
            result += miles.toString().charAt(2) === '1' ? 'Un Mil ' : getCardinalString(miles, result.length > 0) + 'Mil ';
        ...
        if (units > 0) {
            result += units === 1 ? 'Uno' : getCardinalString(units, result);
    ...
    function getCardinalString(digits        , separator        )         {
        ...
        if (tens > 2 && units > 0 && !(digitStringRepresentation.includes('Veinti') || digitStringRepresentation.includes('Dieci'))) {
            digitStringRepresentation += (separator.length > 1) ? 'y ' : 'y ';
        }
        console.log('antes ', separator, units, tens, hundreds);

        digitStringRepresentation += getUnitsCardinalString(units, tens, hundreds, separator.length > 1);
